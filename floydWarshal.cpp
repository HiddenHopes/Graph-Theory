...
void graphshow(int k){
    cout<<"D("<<k<<")\n";
    for(int i=0; i<5; i++){
        for(int j=0; j<5; j++){
            if(graph[i][j]<INF)printf("%3d ",graph[i][j]);
            else printf("*** ");
        }
        cout<<endl;
    }
...
int main() {
  ...
  freopen("floydWarshal4.txt", "r", stdin);
...
//input nicche.
  for (int i = 0; i < E; i++) {    //jotota edge ache totota loop hobe.
    ...
    graph[u-1][v-1] = w; // directed graph hole nicher line baad jabe. graphe teer chinno thakle directed graph.
    //graph[v-1][u-1] = w; //undirected hole ey lineO dite hobe
  }
  graphshow(0);
